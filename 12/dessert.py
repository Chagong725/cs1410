...
class Candy(DessertItem):
    ...
    def can_combine(self, other: "Candy"):
        if isinstance(other, Candy):
            if self.name == other.name and self.price_per == other.price_per:
                return True
        return False

    def combine(self, other: "Candy") -> "Candy":
        if self.can_combine(other):
            return Candy(self.name, self.weight + other.weight, self.price_per)
        else:
...
class Cookie(DessertItem, Freeze):
    ...
    def combine(self, other: "Cookie") -> "Cookie":
        if self.can_combine(other):
            return Cookie(self.name, self.weight + other.weight, self.price_per)
        else:
